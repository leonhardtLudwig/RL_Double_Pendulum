...
def terminated_func(observation):
    ...
    # termina se velocità angolari troppo alte (sistema fuori controllo)
    if abs(dth1) > 15.0 or abs(dth2) > 15.0:
        return True
    return False


def reward_func(observation, action):
    th1  = np.arctan2(observation[1], observation[0])  # [-π, π]
    th2  = np.arctan2(observation[3], observation[2])
    dth1 = observation[4] * max_velocity
    dth2 = observation[5] * max_velocity

    # cos(th1) = -1 quando th1=±π (punto alto) → reward massima = 1
    # cos(th1) = +1 quando th1=0  (punto basso) → reward minima = -1
    r = (
        + (np.cos(th1) - 1.0)          # in [-2, 0], massimo in th1=±π  ← segnale principale
        - 0.5  * th2**2                 # penalizza disallineamento giunto 2
        - 0.05 * (dth1**2 + dth2**2) / max_velocity**2   # penalizza velocità eccessive
        - 0.01 * float(action[0])**2   # penalizza azioni grandi
    )
    return float(r)
...
if __name__ == "__main__":
    agent.learn(total_timesteps=100_000, callback=eval_callback, progress_bar=True)
